...
1. **우선순위 랭킹** — 소스 코드를 lockfile·minified 번들·생성 코드
   ...
   독차지하지 못합니다.
2. **파일 단위 압축(condensing)** — 파일 하나가 `MAX_FILE_DIFF_CHARS`를 넘으면
   앞부분만 남기는 게 아니라 **중요한 hunk를 골라서 남깁니다.** (아래 참고)
...
앞에서부터 자르면 파일 **뒤쪽의 중요한 변경이 통째로 사라집니다.** 특히
대량 치환처럼 앞부분이 반복적인 보일러플레이트일 때 치명적입니다. 그래서
글자 수로 자르지 않고 **hunk 단위로 점수를 매겨 선별**합니다.

**점수 기준** (높을수록 우선 보존)

| 가중치 | 내용 |
|---|---|
| +3.0 | 인증/권한/시크릿, `eval`·`exec`·`subprocess`, 역직렬화, SQL, XSS 관련 |
| +2.0 | 예외 처리, 재시도/타임아웃, 롤백 |
| +2.0 | 동시성 (lock, mutex, async, transaction, race) |
| +1.0 | I/O·네트워크, 함수/클래스 정의 변경, `TODO`/`FIXME` |
| 0.3 이하 | import만 변경(0.3), 주석만 변경(0.2), 공백만 변경(0.05) |

**중복 hunk 접기** — 같은 변경이 반복되면(대량 rename 등) 숫자·문자열
리터럴을 정규화해 동일 패턴으로 묶고, 대표 2개만 남긴 뒤 나머지는 개수로
표시합니다.

**생략 표시** — 생략된 구간은 `~~~` 로 시작하는 주석 라인으로 표시되고,
모델에게 "이건 코드가 아니라 주석이니 코멘트하지 말라"고 프롬프트에
명시합니다.
...
-  log.debug('step 1')
+  log.info('step 1')
~~~ 58 less relevant hunk(s) omitted
@@ -9000,3 +9000,4 @@
-    if not verify_signature(payload, secret):
-        raise AuthError('bad signature')
+    # signature check disabled for now
+    pass
~~~ of those, 58 were near-identical repeats of the same change shown above
```

> 파일 맨 끝에 있던 서명 검증 제거가 살아남았습니다. 단순 앞부분 자르기였다면
> 사라졌을 부분입니다.

**줄 번호 정확성** — `~~~` 마커는 diff 파서에서 무시되므로 줄 번호가
어긋나지 않습니다. 인라인 코멘트는 여전히 **모델이 실제로 본 hunk**의
줄에만 달립니다.
